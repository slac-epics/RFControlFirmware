/****************************************************
 * FWControl_sis8300_eicsys_iqfb.h
 * 
 ...
 ****************************************************/
#ifndef FW_CONTROL_SIS8300_EICSYS_IQFB_H
#define FW_CONTROL_SIS8300_EICSYS_IQFB_H
...
#include "FWControl_sis8300_eicsys_iqfb_board.h"            /* use the functions talking to board */
...
/**
 * Define the data structure for the firmware. Here defines most of data that will be connect to EPICS PVs
...
typedef struct {
    ...
    /* this part define the variables for the build in platform firmware provided by EICSYS. All parameters
     * corresponds to the registers defined in the EICSYS UniDAQ firmware 
     */
	...
	char          board_FPGAType[128];                      /* FPGA type */		
    char          board_platformFwCompileTime[128];         /* time for compiling of the platform firmware */	

    volatile double board_clkMainFreq_MHz;                  /* frequency of on board main clock, for definition, see the EICSYS firmware manual */
    volatile double board_clkExt0Freq_MHz;	
    volatile double board_clkExt1Freq_MHz;	
    volatile double board_clkDDR00Freq_MHz;
    volatile double board_clkDDR90Freq_MHz;
    volatile double board_clkDDRDvFreq_MHz;
    volatile double board_clkIDelayFreq_MHz;
    volatile double board_clkSPIFreq_MHz;
    ...
    volatile unsigned short board_reset;                    /* reset of the board (1 - reset state; 0 - normal operation state) */
    ...
    volatile unsigned short board_IRQEnabled;               /* 1 to indicate the user IRQ (DMA) is enabled */    
    ...
    volatile unsigned short board_DAQShareSel;              /* there are total 16 DAQ channels, the first 10 channels are used by ADCs, the last 6 channels are shared by internal data, this value is to do selection:
                                                               OUP_DAQ_DATA_BUS(10) <= smath_resizeVector(sig_diag_ref_i,     CON_DAQ_DATA_SIZE) when (reg_daq_share_sel = '0') else smath_resizeVector(sig_diag_err_i, CON_DAQ_DATA_SIZE);
                                                               OUP_DAQ_DATA_BUS(11) <= smath_resizeVector(sig_diag_ref_q,     CON_DAQ_DATA_SIZE) when (reg_daq_share_sel = '0') else smath_resizeVector(sig_diag_err_q, CON_DAQ_DATA_SIZE);
                                                               OUP_DAQ_DATA_BUS(12) <= smath_resizeVector(sig_diag_fbk_i,     CON_DAQ_DATA_SIZE) when (reg_daq_share_sel = '0') else smath_resizeVector(sig_diag_act_i, CON_DAQ_DATA_SIZE);
                                                               OUP_DAQ_DATA_BUS(13) <= smath_resizeVector(sig_diag_fbk_q,     CON_DAQ_DATA_SIZE) when (reg_daq_share_sel = '0') else smath_resizeVector(sig_diag_act_q, CON_DAQ_DATA_SIZE);    
                                                               OUP_DAQ_DATA_BUS(14) <= smath_resizeVector(sig_diag_tracked_i, CON_DAQ_DATA_SIZE) when (reg_daq_share_sel = '0') else smath_resizeVector(sig_data_dac_i, CON_DAQ_DATA_SIZE);
                                                               OUP_DAQ_DATA_BUS(15) <= smath_resizeVector(sig_diag_tracked_q, CON_DAQ_DATA_SIZE) when (reg_daq_share_sel = '0') else smath_resizeVector(sig_data_dac_q, CON_DAQ_DATA_SIZE);
                                                            */
    ...
    volatile long board_ADCSamplePno_old;                   /* Temp variable helped to detect the changes of the board_ADCSamplePno */
    ...
    double  board_setPointTable_I[FWC_SIS8300_EICSYS_IQFB_CONST_SP_TAB_BUF_DEPTH];      /* table for set point of I/Q */
    double  board_setPointTable_Q[FWC_SIS8300_EICSYS_IQFB_CONST_SP_TAB_BUF_DEPTH];
    
    double  board_drvRotScaleTable[FWC_SIS8300_EICSYS_IQFB_CONST_DRV_TAB_BUF_DEPTH];    /* driving chain rotation tables */
    double  board_drvRotAngleTable[FWC_SIS8300_EICSYS_IQFB_CONST_DRV_TAB_BUF_DEPTH];

    short board_ADC0_raw[FWC_SIS8300_EICSYS_IQFB_CONST_ADC_SAMPLE_MAX];                 /* ADC raw data for display */
    short board_ADC1_raw[FWC_SIS8300_EICSYS_IQFB_CONST_ADC_SAMPLE_MAX];
    short board_ADC2_raw[FWC_SIS8300_EICSYS_IQFB_CONST_ADC_SAMPLE_MAX];
    short board_ADC3_raw[FWC_SIS8300_EICSYS_IQFB_CONST_ADC_SAMPLE_MAX];
    short board_ADC4_raw[FWC_SIS8300_EICSYS_IQFB_CONST_ADC_SAMPLE_MAX];
    short board_ADC5_raw[FWC_SIS8300_EICSYS_IQFB_CONST_ADC_SAMPLE_MAX];
    short board_ADC6_raw[FWC_SIS8300_EICSYS_IQFB_CONST_ADC_SAMPLE_MAX];
    short board_ADC7_raw[FWC_SIS8300_EICSYS_IQFB_CONST_ADC_SAMPLE_MAX];
    short board_ADC8_raw[FWC_SIS8300_EICSYS_IQFB_CONST_ADC_SAMPLE_MAX];
    short board_ADC9_raw[FWC_SIS8300_EICSYS_IQFB_CONST_ADC_SAMPLE_MAX];
    ...
    double DAQTimeAxis_ns[FWC_SIS8300_EICSYS_IQFB_CONST_ADC_SAMPLE_MAX];       /* time axis in ns for internal DAQ */
    double ADCTimeAxis_ns[FWC_SIS8300_EICSYS_IQFB_CONST_ADC_SAMPLE_MAX];       /* time axis in ns for ADC waveform */

} FWC_sis8300_eicsys_iqfb_struc_data;
